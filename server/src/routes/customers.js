...
export async function registerCustomerRoutes(fastify) {
  // 列表 + 搜索（手机号/名字/会员号）
  fastify.get('/api/customers', async (req) => {
    const { shop_id, q, limit = 50 } = req.query
  ...
  fastify.get('/api/customers/:id', async (req, reply) => {
    const c = fastify.db.prepare(`SELECT * FROM customers WHERE id=?`).get(req.params.id)
  ...
  // 钱包流水
  fastify.get('/api/customers/:id/wallet', async (req) => {
